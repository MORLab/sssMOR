...
for i_Zeile = 1:q
     ...
     for i_Spalte = 1:q
         for i_ref = 1:p
             temp_E(i_ref) = full(sys_ref{i_ref}.E(i_Zeile,i_Spalte));                        
             temp_A(i_ref) = full(sys_ref{i_ref}.A(i_Zeile,i_Spalte));
         end
     ...
     for i_ref = 1:p
         temp_B(i_ref) = full(sys_ref{i_ref}.B(i_Zeile));
         temp_C(i_ref) = full(sys_ref{i_ref}.C(i_Zeile));
     end
