function  [mag, phase, omega] = nicebode(sys, varargin)
...
%         end
        ...
        ylabel('Magnitude /dB','Interpreter',int) 
        ...
        set(gca, 'XScale', 'log');
        set(gca, 'XLim', [min(omega) max(omega)]);
        ylabel('Phase /$^\circ$','Interpreter',int);
        xlabel('Frequency /rad/s','Interpreter',int);
    end
end
