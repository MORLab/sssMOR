function [s0_inp,s0_out,Rt,Lt] = My_initializeShifts(sys,s0_out,Rt,Lt,Opts)
...
Def.nShifts     =10;                     %Number of shifts
Def.nSets       =1;                    %Number of shift vectors 
...
switch Opts.strategy{1}
    ...
    case 'constant'
        s0_inp=Opts.constValue*ones(Opts.nSets,Opts.nShifts);
        
    case 'eigs'
        if isempty(Rt)
            s0_inp=-(eigs(sys,Opts.nShifts*Opts.nSets,Opts.eigsType))';
            ...
            s0_inp = reshape(s0_inp,Opts.nShifts,Opts.nSets)';
        else
        end
    ...
    case 'ADI'
        ...
        if(length(s0_inp)>Opts.nShifts*Opts.nSets)
